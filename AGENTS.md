...
- Languages: en (base), zh (fallback) — the only maintained languages (see Hard Constraints: Chinese-and-English-only i18n). `zh-TW`, `fr`, `ru`, `ja`, `vi` remain as legacy locale files and must not receive new keys.
...
- **Chinese-and-English-only i18n:** The frontend interface maintains only two languages: `en` (base) and `zh` (Simplified Chinese fallback). New or changed UI copy MUST be added only to `web/src/i18n/locales/en.json` and `web/src/i18n/locales/zh.json`. Do NOT propagate new keys into `zh-TW.json`, `fr.json`, `ru.json`, `ja.json`, or `vi.json`; those legacy locale files stay in place but receive no further maintenance. If `bun run i18n:sync` fills missing keys into those files, discard those changes before committing.
