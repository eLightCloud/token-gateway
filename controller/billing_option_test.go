...
func TestPreConsumePolicyDatabaseMatrix(t *testing.T) {
	...
	for _, dialect := range []struct{ kind, env string }{{"sqlite", ""}, {"mysql", "TEST_MYSQL_DSN"}, {"postgres", "TEST_POSTGRES_DSN"}} {
		t.Run(dialect.kind, func(t *testing.T) {
			...
			require.NoError(t, db.AutoMigrate(&model.Token{}, &model.TaskSettlementJournal{}, &model.SystemTask{}, &model.SystemTaskLock{}))
			...
			for i, tc := range cases {
				t.Run(tc.name, func(t *testing.T) {
					...
					user := model.User{Username: fmt.Sprintf("policy-user-%d", i), Quota: int64(tc.wallet), Group: "default", AffCode: fmt.Sprintf("policy-aff-%d", i)}
					require.NoError(t, db.Create(&user).Error)
					token := model.Token{UserId: user.Id, Key: fmt.Sprintf("policy-token-%d", i), RemainQuota: int64(tc.token), UnlimitedQuota: tc.unlimited}
					...
					require.NoError(t, info.Billing.Settle(actual))
					require.NoError(t, db.First(&user, user.Id).Error)
					assert.EqualValues(t, tc.wallet-actual, user.Quota)
					if !tc.unlimited {
						require.NoError(t, db.First(&token, token.Id).Error)
						assert.EqualValues(t, tc.token-actual, token.RemainQuota)
					}
				})
			}
		})
