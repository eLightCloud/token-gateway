...
func TestResponsesWSRequestRunnerRefreshesBillingContextAndCleansBody(t *testing.T) {
	...
	require.Nil(t, runner(request, "ws-first", func(c *gin.Context) *types.NewAPIError {
		...
		assert.EqualValues(t, 100, common.GetContextKeyInt64(c, "token_quota"))
	...
	require.Nil(t, runner(request, "ws-second", func(c *gin.Context) *types.NewAPIError {
		assert.EqualValues(t, 2, common.GetContextKeyInt64(c, "token_quota"))
...
func TestResponsesInterruptedStreamHealth(t *testing.T) {
	for _, scenario := range []string{"websocket timeout", "sse timeout", "sse client cancel"} {
		t.Run(scenario, func(t *testing.T) {
			...
			if scenario == "sse client cancel" {
				// The local SSE drain policy refunds when upstream never supplies
				// a success terminal and authoritative usage after disconnect.
...
func assertResponsesWSAccounting(t *testing.T, fixture *responsesWSBillingTest, expectedQuotas []int) {
	...
	for index, quota := range expectedQuotas {
		assert.EqualValues(t, quota, logs[index].Quota)
		charged += quota
	}
	...
	assert.EqualValues(t, 3000-charged, fixture.token.RemainQuota)
	assert.EqualValues(t, charged, fixture.token.UsedQuota)
	assert.EqualValues(t, 100000-charged, fixture.user.Quota)
	assert.EqualValues(t, charged, fixture.user.UsedQuota)
}
...
func TestResponsesWebSocketReusesConnectionAndSettlesEachRequest(t *testing.T) {
	...
	for index, expectedQuota := range []int{1000, 2000} {
		assert.EqualValues(t, expectedQuota, logs[index].Quota)
	...
	assert.EqualValues(t, 3000, token.UsedQuota)
	assert.EqualValues(t, 97000, user.Quota)
	assert.EqualValues(t, 3000, user.UsedQuota)
}
...
func TestResponsesWebSocketInitialUpstreamRejectionRefundsReservation(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, int64(2000), <-preConsumed, "the rejected request reserved quota before contacting upstream")
