...
管理员直接调额使用主库 `UserQuotaAdjustment` 记录不可变财务事实：`delta_quota` 保存增加、扣减或覆盖产生的有符号余额变化，`balance_before` / `balance_after` 保存事务前后余额，`mode` 保存操作方式，`operator_user_id` 保存操作者。该记录与 `users.quota` 更新在同一事务提交，Invoice 按 `user_id + created_at` 索引汇总账期净额。
