...
database "Main DB\nUser / Organization / Member / Rules" as MainDB
database "LOG_DB\nLog charge facts" as LogDB
component "Billing / Logs views" as Views
component "Invoice calculator" as Invoice
...
OrgService --> Invoice : category x month x factor
...
权威分类目录位于 `model/organization_invoice_category_catalog.go`。它在包初始化时把确认清单构建为“规范化完整模型名 → 类别”索引，并拒绝归一化后重复归属；`organization_invoice.go` 的 Invoice 汇总和已使用类别查询只调用同一分类入口。规范化固定为 `TrimSpace + strings.ToLower`，分类先查完整模型名，再按目录中显式声明的无歧义家族前缀匹配，不使用任意子串推断。
...
Invoice 专用 CSV 只包含组织名称、北京时间账期、币种、相互对齐的真实姓名与账号表头行、交叉表金额、结算系数、结算后金额、当期成功充值订单金额和导出时余额，不包含组织 ID 元数据、内部时间戳、规则来源、原始 quota、日志内容或请求标识。它与 GET Invoice 复用同一消费聚合服务；授权通过后，导出路径再从主数据库读取未脱敏的财务补充字段，页面与 JSON API 的脱敏合同保持不变。

财务补充投影遵守以下边界：

- 账号集合以 `invoice.Accounts` 为准；`users.username` 是稳定账号表头，`display_name` 只用于真实姓名行。用户、余额和充值订单均按这批账号批量查询，禁止逐用户 N+1；账号在主库中缺失时整个导出失败，不以零值掩盖数据不一致。
- 当前余额直接读取主数据库 `users.quota`，不读 Redis，并按 `quota / QuotaPerUnit` 换算；它表达导出瞬间的余额，不回溯账期末快照。
- 成功充值订单使用 `status=success`、`complete_time >= start_timestamp`、`complete_time < end_timestamp + 1` 和支付渠道白名单筛选，并通过数据库侧 `NOT EXISTS` 与 `subscription_orders.trade_no` 排除订阅订单。
- 支持渠道按现有入账语义还原 USD：Stripe 使用 `Money`，Waffo/Waffo Pancake 使用 `Amount`，Creem 使用 `Amount / QuotaPerUnit`。金额用 decimal 累加；已知渠道的异常字段被排除并写服务端告警，未知渠道不猜测。
- 管理员直接增加、扣减或覆盖额度时，余额更新与不可变 `user_quota_adjustments` 流水在主库同一事务提交；`delta_quota` 分别记录正数、负数或新旧余额差额，Invoice 按 `created_at` 半开区间汇总并换算 USD。
- 新表只提供上线后持久化事实；上线前直接调额必须来自可信原始数值回填。旧审计日志中的格式化额度文案受展示币种影响，不能作为自动回填依据，也不能将缺失流水解释为零调额。
- EPay 因正常成功路径没有可靠 `complete_time` 而统一排除；不得用 `create_time`、日志文本或订单号推断账期和金额。兑换码、退款、奖励及其他非订单余额变化也不属于该净额。

这些查询都是 Invoice 专用只读投影；直接调额流水是管理员余额变更链路的最小财务事实，不替代完整钱包总账，也不进入充值、消费、预扣、结算、退款或订阅链路。消费聚合继续读取日志数据库，身份、充值、调额和余额继续读取主数据库，保持双数据库职责分离。

## 迁移与兼容性

`Organization`、`OrganizationMember` 和 `OrganizationBillingSettlementRule` 已进入普通迁移与快速迁移列表；迁移会新增 `billing_start_at`、创建结算规则表，并把旧 `owner`/`billing` 角色归一为 `admin`/`member`。旧成员的 `billing_start_at = 0` 不做自动历史回填，查询时回退 `joined_at`。所有查询需继续兼容 SQLite、MySQL、PostgreSQL；日志查询还要保持 ClickHouse 分支。
