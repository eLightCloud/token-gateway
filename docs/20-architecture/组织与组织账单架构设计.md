...
- EPay 因正常成功路径没有可靠 `complete_time` 而统一排除；不得用 `create_time`、日志文本或订单号推断账期和金额。兑换码、退款、奖励、管理员调额及其他非订单余额变化也不属于“成功充值订单金额”。

这些查询都是 Invoice 专用只读投影，不新增迁移、索引或钱包总账，也不进入充值、余额变更、消费、预扣、结算、退款或订阅链路。消费聚合继续读取日志数据库，身份、充值和余额继续读取主数据库，保持双数据库职责分离。
