...
- 新表提供上线后持久化调额事实。对上线前的正向调额，期初基线和月度汇总的后台构建兼容解析已知的 `user.quota_add` 结构、更早的“管理员增加用户额度”中文审计结构和严格 USD 金额格式，并复用组织成员归属边界。具有新 `quota_value/adjustment_id` 的日志不走兼容路径；已固化到历史事实表的日志也不重复计算。精确匹配但无法解析的日志必须失败关闭，不得当作零调额。该兼容查询只在基线缺失或缺少 `ready` 汇总时的后台构建中执行。
