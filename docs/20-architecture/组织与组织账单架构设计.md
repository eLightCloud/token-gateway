...
Invoice 专用 CSV 只包含组织名称、北京时间账期、币种、相互对齐的真实姓名与账号表头行、交叉表金额、结算系数和结算后金额，不包含组织 ID 元数据、内部时间戳、规则来源、原始 quota、日志内容或请求标识。它与 GET Invoice 复用同一聚合服务；授权通过后，导出路径再从主数据库读取组织名称和未脱敏 `display_name`，真实姓名缺失时输出空单元格，不改变页面与 JSON API 的脱敏合同。
