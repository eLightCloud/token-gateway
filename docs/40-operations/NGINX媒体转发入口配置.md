...
> 本文给出“视频下载经渠道 API 地址转发”的 NGINX 媒体入口参考配置与验收清单，实现 `docs/80-dev/2026-09-22-豆包视频南向协议扩展方案.md` 第 5 节定义的出站合同。**宿主与参考配置已通过本地验证，但这不是可直接照抄的生产配置**：域名、证书、DNS、访问控制必须按实际环境替换；线上真实 NGINX 与 HK 出口验证通过前，不得开启生产渠道的媒体转发开关。
