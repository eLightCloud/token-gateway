...
User | Role | Joined at | Billing start | Status | Actions
```

- Add member 先搜索用户，再选择角色。
- 角色选项固定为 Admin、Member。
- Include removed 展示历史成员和离开状态。
- Billing start 显示当前生效的账单归属起点；旧数据为零时回退显示 Joined at，不显示 1970 或空值。
- Calendar 操作打开“调整账单归属起点”对话框：先选择北京时间日期并 Preview，再展示新增请求数、quota、日志范围、最早保留日志和窗口冲突；只有无冲突且预览仍匹配当前候选值时才允许 Apply。
- 对话框明确说明该操作只影响组织报表，不改变个人钱包或已经完成的扣费；应用成功后刷新成员、Billing、Logs 和 Invoice 相关查询。
