...
func TestPreviewAndApplyLegacyUserQuotaAdjustmentsIsScopedAndIdempotent(t *testing.T) {
	...
	assert.Equal(t, int64(8001), candidates[0].SourceLogId)
	assert.Equal(t, "1500.000000", candidates[0].AmountUSD)
	assert.Equal(t, int64(750_000_000), candidates[0].DeltaQuota)
	assert.False(t, candidates[0].AlreadyApplied)
	assert.Equal(t, int64(8004), candidates[1].SourceLogId)
	assert.Equal(t, "300.000000", candidates[1].AmountUSD)
	assert.Equal(t, int64(150_000_000), candidates[1].DeltaQuota)
