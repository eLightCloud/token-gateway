...
func TestApplyAdminUserQuotaAdjustmentPersistsExactWalletDeltas(t *testing.T) {
	...
	assert.EqualValues(t, 1_000_000, added.PreviousQuota)
	assert.EqualValues(t, 1_500_000, added.CurrentQuota)
	assert.EqualValues(t, 500_000, added.DeltaQuota)

	subtracted, err := ApplyAdminUserQuotaAdjustment(20, 1, UserQuotaAdjustmentModeSubtract, 250_000)
	require.NoError(t, err)
	assert.EqualValues(t, -250_000, subtracted.DeltaQuota)

	overridden, err := ApplyAdminUserQuotaAdjustment(20, 1, UserQuotaAdjustmentModeOverride, 2_000_000)
	require.NoError(t, err)
	assert.EqualValues(t, 750_000, overridden.DeltaQuota)
	assert.EqualValues(t, 2_000_000, overridden.CurrentQuota)

	var storedUser User
	require.NoError(t, DB.Select("quota").First(&storedUser, 20).Error)
	assert.EqualValues(t, 2_000_000, storedUser.Quota)
	...
	assert.EqualValues(t, []int64{500_000, -250_000, 750_000}, []int64{
...
func TestApplyAdminUserQuotaAdjustmentRejectsOverflowWithoutPartialWrite(t *testing.T) {
	...
	assert.EqualValues(t, common.MaxWalletQuota, stored.Quota)
	var count int64
