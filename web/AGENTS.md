...
- **语言范围（硬约束）**：界面语言只维护英文（`en`，基准）与简体中文（`zh`，回退）两种。新增或修改的文案只需且只准写入 `src/i18n/locales/en.json` 与 `src/i18n/locales/zh.json`；`zh-TW/fr/ru/ja/vi` 为遗留语言包，保留但不再维护、不新增 key。`bun run i18n:sync` 若向遗留语言包补齐了缺失 key，提交前须丢弃这些改动。
...
- **2026-09-23**：在 3.1 中补充「语言范围」硬约束：界面语言仅维护 en/zh，遗留语言包不再新增 key。
